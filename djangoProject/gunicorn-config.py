...
workers = multiprocessing.cpu_count() * 2 + 1  # 并行工作进程数, int，cpu数量*2+1 推荐进程数
...
worker_class = 'gevent' # 工作模式协程，默认的是sync模式

worker_connections = 65535 # 设置最大并发量（每个worker处理请求的工作线程数，正整数，默认为1）
# 最大客户端并发数量，默认情况下这个值为1000。此设置将影响gevent和eventlet工作模式
max_requests = 2000 # 设置进程文件目录
...
graceful_timeout = 10 # 超时重启

keepalive = 60 # 在keep-alive连接上等待请求的秒数，默认情况下值为2。一般设定在1~5秒之间。
